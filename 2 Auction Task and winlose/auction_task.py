...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\mauspad\\Desktop\\new_OEA_stuff\\OEA_task_test\\auction\\2 Auction Task and winlose\\auction_task.py',
...
fix_pause = visual.ShapeStim(
    win=win, name='fix_pause', vertices='cross',
    size=(65,65),
    ori=0, pos=(0, 0),
    lineWidth=1, lineColor='Black', lineColorSpace='rgb',
    fillColor='Black', fillColorSpace='rgb',
...
for thisBid_item in bid_items:
    ...
    while continueRoutine:
        ...
        if fix.status == STARTED:
            ...
            if tThisFlipGlobal > fix.tStartRefresh + jitter-frameTolerance:
    ...
    for thisComponent in fixationComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    bid_items.addData('fix.started', fix.tStartRefresh)
    bid_items.addData('fix.stopped', fix.tStopRefresh)
    ...
    while continueRoutine:
        ...
        # *fix_pause* updates
        if fix_pause.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            fix_pause.frameNStart = frameN  # exact frame index
            fix_pause.tStart = t  # local t and not account for scr refresh
            fix_pause.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(fix_pause, 'tStartRefresh')  # time at next scr refresh
            fix_pause.setAutoDraw(True)
        if fix_pause.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fix_pause.tStartRefresh + jitter-frameTolerance:
                # keep track of stop time/frame for later
                fix_pause.tStop = t  # not accounting for scr refresh
                fix_pause.frameNStop = frameN  # exact frame index
                win.timeOnFlip(fix_pause, 'tStopRefresh')  # time at next scr refresh
                fix_pause.setAutoDraw(False)
    ...
    for thisComponent in pauseComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    bid_items.addData('fix_pause.started', fix_pause.tStartRefresh)
    bid_items.addData('fix_pause.stopped', fix_pause.tStopRefresh)
